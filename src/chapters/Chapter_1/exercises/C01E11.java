package chapters.chapter_1.exercises;

public class C01E11 {
	public static void main(String[] args) {
		System.out.println("Numbers of the seconds ain a year: " + (60 * 60 * 24 * 365));
		System.out.println("Numbers of the birth per yaer: " + (31536000 / 7.0));
		System.out.println("Numbers of the death per yaer: " + (31536000 / 13.0));
		System.out.println("Numbers of the immigrants per yaer: " + (31536000 / 45.0));
		
		System.out.println("Year\t\t\tPopulatin" + 
		"\n0\t\t\t\t312,032,486" + 
				"\n1\t\t\t\t" + (312032486 + 4505142.857142857 - 2425846.153846154 + 700800.0) + 
				"\n2\t\t\t\t" + (3.148125827032967E8 + 4505142.857142857 - 2425846.153846154 + 700800.0) +
				"\n3\t\t\t\t" + (3.1759267940659344E8 + 4505142.857142857 - 2425846.153846154 + 700800.0) +
				"\n4\t\t\t\t" + (3.2037277610989016E8 + 4505142.857142857 - 2425846.153846154 + 700800.0) +
				"\n5\t\t\t\t" + (3.231528728131869E8 + 4505142.857142857 - 2425846.153846154 + 700800.0));
	}

	

}
